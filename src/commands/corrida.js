...
ProgressBar = require('../utils/progress');
            ...
            for (let i = 0; i < random(1, 5); i++) {
              ...
              if (bot.lastDraw) {
                ...
                if (bot.complete && !user.complete) {
                  result += 'Acabada - Ganhador ' + client.user.username;
                  ...
                  result += 'Acabada - Ganhador ' + message.author.username;
                  progressUser = `:vertical_traffic_light: ${user.lastDraw}:checkered_flag: :trophy:`;
                  addValue(client, message.author.id, betValue * random(1, 3) - betValue)
                }
                else if (bot.complete && user.complete) {
                  result += 'Acabada - Empate';
                }
              ...
              if (nunket.complete || racer.complete) {
                clearInterval(timer);
              }
            }, 1000);
    },
